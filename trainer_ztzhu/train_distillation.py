...
def train_epoch(
    ...
    for step, (X, Y, loss_mask) in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == iters - 1:
            ...
            Logger(
                f"Epoch:[{epoch+1}/{args.epochs}]({step}/{iters}) loss:{current_loss:.6f} ce:{ce_loss.item():.4f} distill:{distill_loss.item():.4f} lr:{current_lr:.12f} epoch_Time:{eta_min}min:"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--from_student_weight", default="full_sft", type=str, help="学生模型基于哪个权重"
    )
    parser.add_argument(
        "--from_teacher_weight", default="full_sft", type=str, help="教师模型基于哪个权重"
    )
    parser.add_argument(
        "--from_resume", default=0, type=int, choices=[0, 1], help="是否自动检测&续训（0=否，1=是）"
    )
    parser.add_argument(
        "--alpha", default=0.5, type=float, help="CE损失权重，总损失=alpha*CE+(1-alpha)*KL"
    )
    parser.add_argument(
